...
def entrenar_y_evaluar_modelo_regresion_completo(ruta_excel):
    try:
        # 1. Cargar los datos desde el enlace de Google Drive (formato XLSX)
    ...
    # Convertir 'periodo' a tipo numérico y eliminar filas con NaN en 'periodo' o 'punt_global'
    ...
    # Se han revisado los mapeos para asegurar que cubren todos los posibles resultados.
    mapeos_ordinales = {
        "estu_valormatriculauniversidad": {
    ...
    # --- NUEVAS ESTRATEGIAS DE INGENIERÍA DE CARACTERÍSTICAS ---
    print("\n--- Aplicando Ingeniería de Características Avanzada ---")

    # 1. Tratamiento de Outliers con Winsorización [1, 2]
    # NOTE: 'percentil_nbc' is not in your selected columns.
    # If you intend to use it, you must add it to 'columnas_seleccionadas'
    # or ensure it exists in your DataFrame.
    columnas_para_winsorizar = [] # Removed 'percentil_nbc' as it's not selected
    ...
    # 2. Transformaciones No Lineales (Logarítmica) [3, 4]
    # NOTE: 'percentil_nbc' is not in your selected columns.
    columnas_para_log_transformar = [] # Removed 'percentil_nbc' as it's not selected
    for col in columnas_para_log_transformar:
        if col in df_seleccionado.columns:
            # Add a small constant to avoid log(0) or log(negative numbers)
            df_seleccionado[col] = np.log(df_seleccionado[col] + 1e-6)
            print(f"Columna '{col}' transformada logarítmicamente.")

    # 3. Creación de Características Polinómicas [5, 6, 7, 8]
    poly = PolynomialFeatures(degree=2, include_bias=False)
    
    # Ensure these columns exist before trying to create polynomial features
    ...
    if numeric_cols_for_poly:
        # Impute NaNs before PolynomialFeatures, as it doesn't handle them
        ...
        print(f"Nuevas columnas polinómicas (muestra): {df_poly_features.columns.tolist()[:5]}...") # Print only first 5
    else:
        print("No se encontraron columnas adecuadas para características polinómicas.")


    # 4. Filtrar por período para 80% entrenamiento y 20% prueba
    ...
    if df_entrenamiento.empty or df_prueba.empty:
        print("Error: No hay suficientes datos para los períodos de entrenamiento o prueba especificados después del filtrado.")
        return None

    print(f"Filas para entrenamiento: {len(df_entrenamiento)}")
    print(f"Filas para prueba: {len(df_prueba)}")

    # 5. Manejo de variables categóricas BINARIAS (One-Hot Encoding)
    ...
    # --- Imputación Avanzada de NaN y valores infinitos con KNNImputer [9, 10] ---
    ...
    print("\nValores nulos en X_train y X_test imputados con KNNImputer.")
    ...
    if X_train.empty or X_test.empty or y_train.empty or y_test.empty:
        print("Error: Los conjuntos de datos X o y están vacíos después del preprocesamiento y la imputación.")
        return None

    # --- Análisis de Multicolinealidad con VIF [11, 12, 13, 14] ---
    print("\n--- Análisis de Multicolinealidad (Factor de Inflación de la Varianza - VIF) ---")
    X_train_vif = sm.add_constant(X_train, has_constant='add') # Ensure constant is added if not already
    
    # Drop any non-numeric columns that might have slipped through for VIF calculation
    X_train_vif_numeric = X_train_vif.select_dtypes(include=np.number)
    
    # Drop rows with NaN if any exist after constant addition (VIF cannot handle NaNs)
    X_train_vif_numeric = X_train_vif_numeric.dropna()

    vif_data = pd.DataFrame()
    vif_data["feature"] = X_train_vif_numeric.columns
    
    # CORRECTED LINE: Use shape[1] for number of columns
    vif_data["VIF"] = [variance_inflation_factor(X_train_vif_numeric.values, i) for i in range(X_train_vif_numeric.shape[1])]

    print(vif_data.sort_values(by="VIF", ascending=False))
    print("\nNota: Un VIF > 5 o 10 indica multicolinealidad severa. Considerar eliminar o combinar variables.")

    # --- Entrenamiento y Evaluación de Modelos ---

    resultados_modelos = {} # Diccionario para almacenar resultados de cada modelo

    # 1. Modelo de Regresión Lineal Múltiple (OLS) con características mejoradas
    print("\n--- Entrenamiento y Evaluación del Modelo de Regresión Lineal Múltiple (OLS) ---")
    ...
    resultados_modelos['OLS'] = {'R^2': r2_ols, 'RMSE': rmse_ols} # Storing as a dict for consistency
    ...
    print("\n--- Fórmula de Regresión Lineal (Modelo OLS) ---")
    ...
    # 2. Modelo de Regresión Lasso (Regularización L1) [16, 17, 18, 19]
    print("\n--- Entrenamiento y Evaluación del Modelo de Regresión Lasso ---")
    lasso_model = Lasso(alpha=0.1, random_state=42) # alpha can be adjusted
    ...
    # 3. Modelo XGBoost Regressor (Base) [20, 21, 22, 23]
    print("\n--- Entrenamiento y Evaluación de XGBoost Regressor (Base) ---")
    ...
    print("\n--- Ajuste de Hiperparámetros para XGBoost con RandomizedSearchCV ---")
    param_grid_xgboost = {
        'n_estimators': [300, 500, 700, 1000, 1500], # Increased estimators
        'learning_rate': [0.01, 0.05, 0.1, 0.15], # More granular learning rates
        'max_depth': [3, 4, 5, 6, 7, 8], # Deeper trees
        ...
        'reg_alpha': [0, 0.005, 0.01, 0.05], # L1 regularization term on weights
        'reg_lambda': [0, 0.005, 0.01, 0.05] # L2 regularization term on weights
    }
    try:
        random_search_xgboost = RandomizedSearchCV(estimator=XGBRegressor(random_state=42, n_jobs=-1),
                                                 ...
                                                 n_iter=100, # Number of parameter settings that are sampled (can be increased)
                                                 scoring='r2',
        ...
        print(f"R^2 del Modelo XGBoost (Ajustado): {r2_best_xgboost:.2f}")
        print(f"RMSE del Modelo XGBoost (Ajustado): {rmse_best_xgboost:.2f}")
    except Exception as e:
        print(f"\nError durante RandomizedSearchCV para XGBoost: {e}")
        print("Considera reducir el 'param_grid_xgboost' o 'n_iter' o comentar esta sección si los errores de memoria persisten.")


    # --- Gráficas de Diagnóstico del Modelo Lineal (con Statsmodels) ---
    # Estas gráficas son principalmente para el modelo lineal, pero son útiles para entender los datos.
    # Note: Statsmodels OLS model will handle the 'constant' automatically if added using sm.add_constant.
    # We use the imputed X_train and y_train directly.
    ...
    try:
        ...
        # 1. Gráfica de Residuos Estandarizados vs. Valores Ajustados [30, 31]
        ...
        plt.title('Gráfica de Residuos Estandarizados vs. Valores Ajustados (Modelo OLS)')
        plt.grid(True)
        plt.show()

        # 2. Gráfica de Cuantiles Normales (Q-Q Plot) [32]
        plt.figure(figsize=(8, 6))
        stats.probplot(residuals_standardized, dist="norm", plot=plt)
        plt.title('Gráfica de Cuantiles Normales (Q-Q Plot) de Residuos (Modelo OLS)')
        plt.show()

    except Exception as e:
        print(f"\nError al generar el resumen de Statsmodels o las gráficas de diagnóstico: {e}")
        print("Puede haber problemas con la dimensionalidad de los datos o multicolinealidad severa. Asegúrate de que no haya NaN en X_train_sm para Statsmodels.")

    print("\n--- Resumen de Métricas de Rendimiento de Modelos ---")
...
if resultados:
    print("\n¡Análisis completado! Se han generado las métricas y las gráficas de diagnóstico.")
else:
    print("\nEl análisis no pudo completarse debido a los errores mencionados anteriormente.")